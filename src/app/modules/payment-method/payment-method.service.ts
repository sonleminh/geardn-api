...
export class PaymentMethodService {
  ...
  async create(createPaymentMethodDTO: CreatePaymentMethodDto) {
    try {
      ...
      return await this.paymentMethodModel.create(payload);
    } catch (error) {
      throw error;
    }
  }
  ...
  async findAll() {
    try {
      const [res, total] = await Promise.all([
        this.paymentMethodModel.find().lean().exec(),
        this.paymentMethodModel.countDocuments(),
      ]);
      return { PaymentMethodList: res, total };
    } catch (error) {
